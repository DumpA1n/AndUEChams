// CoreUObject helper bodies. Mirrors
// source/UEProber/UECore/CoreUObject_functions.cpp plus the
// ProcessEvent dispatch body for every UFunction in CoreUObject.
//
// Compile and link this .cpp into your TU(s). The bodies depend
// on FName::s_NameResolver and UObject::GObjects being wired
// from your bridge once at startup.
...
// Mirrors UECore CoreUObject_functions.cpp — bodies for UObject helpers
...
struct UFunction* UClass::GetFunction(const std::string& ClassName, const std::string& FuncName) const
{
    for (const struct UStruct* Clss = this; Clss; Clss = Clss->SuperStruct)
    {
        ...
        {
            if (Field->HasTypeFlag(EClassCastFlags{0x80000}) && Field->GetName() == FuncName)
